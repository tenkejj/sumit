...
: "${SERVER:?Set SERVER in deploy/deploy.env or deploy.env in project root (see deploy/deploy.env.example)}"
: "${SSH_KEY:?Set SSH_KEY in deploy/deploy.env or deploy.env in project root (see deploy/deploy.env.example)}"

readonly REMOTE_APP_DIR="/opt/sumit"
readonly BINARY_NAME="sumit"
readonly PROD_ENV_KEYS=(GROQ_API_KEY STATS_KEY)
...
if systemctl is-active --quiet sumit; then
  ...
  echo "Usługa sumit nie jest jeszcze włączona — uruchom jednorazową konfigurację z deploy/DEPLOY.md"
fi
EOF
